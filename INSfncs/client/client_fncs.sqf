...
INS_intro = {
	...
	_camPos = [(getPos player select 0) + _rx, (getPos player select 1) + _ry, (getTerrainHeightASL (position player)) + 800];
	...
	//player sideChat localize "Info_intro";
	waitUntil {camcommitted _cam};
	...
	if (INS_full_loadout isEqualTo 1) then {player sideChat localize "STR_BMR_Reload_toSave_Kit"};
...
*/
//Fonction pour drop un UGV BLUFOR
JIG_UGVdrop_Blu4_fnc = {
	...
	*/
	...
	ghst_ugvsupport = [(markerPos "ugv_spawn"),"maestro_B_UGV_01_rcws_F",1,30] execVM "scripts\ghst_ugvsupport_Blu4.sqf";
	true
};
//Fonction pour drop un UGV OPFOR
JIG_UGVdrop_Op4_fnc = {
	// Player action UGV para drop. by Jigsor
	private _p = _this # 1;
	/*// Require UAV backpack
	...
	*/
	if !({_x find "_UavTerminal" != -1} count assignedItems _p > 0) then {
		if ({_x in ["ItemGPS"]} count assignedItems _p > 0) then {_p unlinkItem "ItemGPS"};
		if ({_x in ["B_UavTerminal"]} count assignedItems _p > 0) then {_p unlinkItem "B_UavTerminal"};
		if ({_x in ["I_UavTerminal"]} count assignedItems _p > 0) then {_p unlinkItem "I_UavTerminal"};
		_p linkItem "O_UAVTerminal";
	}else{
		_p unlinkItem "O_UAVTerminal";
		_p linkItem "O_UAVTerminal";
	};
	ghst_ugvsupport = [(markerPos "ugv_spawn"),"O_UGV_01_rcws_F",1,30] execVM "scripts\ghst_ugvsupport_Op4.sqf";
	true
};
/*
...
JIG_p_actions_resp = {
	...
	*/
	...
	if !(((toLowerANSI (speaker player)) isEqualTo 'novoice') && {_playertype isEqualTo INS_op4_players}) then {
...
JIG_intel_found = {
	...
	_intelMkr setMarkerType "hd_dot";
...
JIG_intel_found_objectif = {
	// Remove intel addaction, grab intel animation, delete intel object, creates intel maker, update JIP intel marker state, global sidechat player name found intel, add 2 points to caller by Jigsor
	params ["_this","_caller","_id"];
	_pName = name _caller;
	_text = format [localize "STR_BMR_found_intel",_pName];
...
Op4_spawn_pos = {
	...
	if (!isNil "_random_w_player") then	{
		...
		_cooX = _playerPos # 0;
		_cooY = _playerPos # 1;
		_wheX = [0,0] call BIS_fnc_randomInt;
		_wheY = [0,0] call BIS_fnc_randomInt;
...
INS_Recruit_skill = {
	...
	params ["_unit"];
		...
		_unit setSkill ["aimingShake",(0.40 + (random 0.1)) ];  
		_unit setSkill ["spotDistance",(0.50 + (random 0.1))];  
		_unit setSkill ["spotTime",0.8];  
		_unit setSkill ["courage",1.0];   
		_unit setSkill ["aimingSpeed",(0.7 + (random 0.3))];
	if (_unit isEqualTo (leader group _unit)) then {_unit setSkill ["commanding", 1];}else{	_unit setSkill ["commanding", 1];};
	true
};
...
INS_RespawnLoadout = {
	...
	missionNamespace setVariable ["INS_SavedLoadout", (getUnitLoadout player)];
	//_kitSaved = getMissionPath "Sound\Prout.ogg";
	//_kitSaved = "A3\Sounds_F\sfx\Beep_Target.wss";
	playSound3D [getMissionPath "Sound\Prout.ogg", player];
