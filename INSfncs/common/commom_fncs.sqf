...
fnc_mp_intel = {
	...
	if (!isNull _intelobj) then {
		_intelobj addAction [("<t color='#ffff00'>") + (localize "STR_BMR_GrabIntel") + "</t>", "call JIG_intel_found", _cachepos, 8];
	};
	true
};
INS_end_mssg = {
...
mhq_obj_fnc = {
	...
	switch (true) do {
		...
		case (_var isEqualTo "RPIMA_RHIB_1") : {_obj = RPIMA_RHIB_1; if (vehicleVarname RPIMA_RHIB_1 isEqualTo "") then {RPIMA_RHIB_1 setVehicleVarname "RPIMA_RHIB_1"; RPIMA_RHIB_1 = _obj}};
		case (_var isEqualTo "RPIMA_RHIB_2") : {_obj = RPIMA_RHIB_2; if (vehicleVarname RPIMA_RHIB_2 isEqualTo "") then {RPIMA_RHIB_2 setVehicleVarname "RPIMA_RHIB_2"; RPIMA_RHIB_2 = _obj}};
		...
		case (_var isEqualTo "INS_RHIB_1") : {_obj = INS_RHIB_1; if (vehicleVarname INS_RHIB_1 isEqualTo "") then {INS_RHIB_1 setVehicleVarname "INS_RHIB_1"; INS_RHIB_1 = _obj}};
		case (_var isEqualTo "INS_SDV_1") : {_obj = INS_SDV_1; if (vehicleVarname INS_SDV_1 isEqualTo "") then {INS_SDV_1 setVehicleVarname "INS_SDV_1"; INS_SDV_1 = _obj}};
		case (_var isEqualTo "") : {};
		default {};
	};
	_obj
...
INS_BluFor_Siren = compileFinal " if (isServer) then {
	[INS_BASE_BLUFOR_Siren,""siren""] call mp_Say3D_fnc;
	[""Ennemie attaque la base défendez la base!""] remoteExec [""JIG_MPsideChatWest_fnc"", [0,-2] select isDedicated];
	alarm1On = false;
}; ";
switchMoveEverywhere = compileFinal " _this select 0 switchMove (_this select 1); ";
INS_Opfor_Siren = compileFinal " if (isServer) then {
	[INS_BASE_OPFOR_Siren,""sirenAncienne""] call mp_Say3D_fnc;
	[""Ennemie attaque la base défendez la base""] remoteExec [""JIG_MPsideChatEast_fnc"", [0,-2] select isDedicated];
	alarm3On = false;
}; ";
