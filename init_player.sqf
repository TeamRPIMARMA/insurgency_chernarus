...
if (DebugEnabled > 0) then {
	...
	0 spawn {
		...
		if (local player) then {
			...
			player addAction[("<t color='#ff1111'>") + (localize "STR_BMR_open_VA") + "</t>",{[player, player, true] call BIS_fnc_arsenal;}]; //"Open"  ["Open",true] call BIS_fnc_arsenal
...
*/
//Active ou désactive spécialité slot paradropt
	//if (Popo_slot_paradrop isEqualTo 1) then {0=[] execVM "onPlayerRespawn.sqf"};
	if (_playertype in INS_W_PlayerUAVop) then {player setVariable ["ghst_ugvsup", 0]};
	if (_playertype in INS_O_PlayerUAVop) then {player setVariable ["ghst_ugvsup", 0]};
/*
...
*/
	...
	//if (INS_ACE_core) then {[Popo_E_VA, true] call ace_arsenal_fnc_initBox};
/*
	...
	if (INS_VA_type in [1,2]) then {
		INS_Wep_box addAction[("<t size='1.5' shadow='2' color='#ff1111'>") + (localize "STR_BMR_open_VA") + "</t>",{[_this] call JIG_VA}, nil, 1.5, true, true, "", "true", 15];//JIG_VA
...
*/
	...
	if (INS_full_loadout isEqualTo 2) then {
		INS_Wep_box addAction[("<t size='1.5' shadow='2' color='#ff9207'>") + (localize "STR_BMR_save_loadout") + "</t>", {call INS_RespawnLoadout}, [], 10, false, true, "", "side _this != EAST", 15];
	};
	
	// Op4 MHQ
	Opfor_MHQ addAction[("<t color='#12F905'>") + ("Deploy MHQ") + "</t>","scripts\deployOpforMHQ.sqf",nil,10, false, true, "", "side _this != INS_Blu_side", 12];
	
	// Op4 Weapon Box
	//if (INS_VA_type in [2,3]) then {INS_E_HQ addAction[("<t size='1.5' shadow='2' color='#ff1111'>") + (localize "STR_BMR_open_VA") + "</t>",{[_this] call JIG_VA}, [], 1, false, true, "", "side _this != INS_Blu_side", 12]};//JIG_VA
	if (INS_full_loadout isEqualTo 2) then {INS_E_HQ addAction[("<t size='1.5' shadow='2' color='#ff9207'>") + (localize "STR_BMR_save_loadout") + "</t>",{call INS_RespawnLoadout}, [], 1, false, true, "", "side _this != INS_Blu_side", 12]};
	//INS_E_HQ addAction[("<t size='1.5' shadow='2' color='#ff1111'>") + (localize "STR_BMR_load_saved_loadout") + "</t>",{(_this select 1) call INS_RestoreLoadout},nil,1, false, true, "", "side _this != INS_Blu_side", 12];
	//INS_E_HQ addAction[("<t size='1.5' shadow='2' color='#12F905'>") + (localize "STR_BMR_restore_default_loadout") + "</t>",{call Op4_restore_loadout},nil,1, false, true, "", "side _this != INS_Blu_side", 12];

	// AI recruitment
	if (max_ai_recruits > 1) then {INS_Wep_box addAction[("<t size='1.5' shadow='2' color='#1d78ed'>") + (localize "STR_BMR_recruit_inf") + "</t>","bon_recruit_units\open_dialog.sqf", [], 10, true, true, "", "true", 15];};
...
*/
	...
	if (DebugEnabled isEqualTo 0) then {
		If (side player == east) then {
			...
			0 spawn {
				...
				*/
				...
				if (INS_full_loadout in [1,2]) then {loadout = getUnitLoadout player};

				if (JIG_MHQ_enabled) then {
					private ["_mhqObj","_mhqPos","_mhqObj1","_mhqObj2"];
					...
					_mhqObj1 = ["INS_RHIB_1"] call mhq_obj_fnc;
					_mhqPos = getPos INS_RHIB_1; _nul = [_mhqObj1,_mhqPos] spawn INS_MHQ_mkr;
					_mhqObj2 = ["INS_SDV_1"] call mhq_obj_fnc;
					_mhqPos = getPos INS_SDV_1; _nul = [_mhqObj2,_mhqPos] spawn INS_MHQ_mkr;
					//INS_E_HQ addAction["<t size='1.5' shadow='2' color='#ED2744'>Transfer to Opfor_MHQ</t>", "call JIG_transfer_fnc", ["Opfor_MHQ"], 1];
					//INS_E_HQ addAction ["<t size='1.5' color='#ED2744'>Aller dans le MHQ</t>","scripts\Teleporter_Opfor_MHQ.sqf"];
					//INS_E_HQ addAction ["<t size='1.5' shadow='2' color='#12F905'>Défendre l'Imam</t>","scripts\Teleporter_Imam.sqf"];
					//INS_E_HQ addAction ["<t size='1.5' shadow='2' color='#12F905'>Aller dans le KA60</t>","scripts\Teleporter_KA60.sqf"];
					//INS_E_HQ addAction ["<t size='1.5' shadow='2' color='#12F905'>Aller dans Pandur</t>","scripts\Teleporter_Pandur.sqf"];
					//INS_E_HQ addAction ["<t size='1.5' shadow='2' color='#12F905'>Aller dans Camion citerne</t>","scripts\Teleporter_CamionCiterne.sqf"];
				};
				//if (!alive KA60) then INS_E_HQ removeaction 3;
		...
		If (side player == west) then {
			...
			0 spawn {
				...
				if (JIG_MHQ_enabled) then {
					private ["_op4","_mhqPos","_mhqObj1","_mhqObj2","_mhqObj3","_mhqObj4","_mhqObj5"];
					...
					_mhqObj4 = ["RPIMA_RHIB_1"] call mhq_obj_fnc;
					_mhqPos = getPos RPIMA_RHIB_1; _nul = [_mhqObj4,_mhqPos] spawn INS_MHQ_mkr;
					_mhqObj5 = ["RPIMA_RHIB_2"] call mhq_obj_fnc;
					_mhqPos = getPos RPIMA_RHIB_2; _nul = [_mhqObj5,_mhqPos] spawn INS_MHQ_mkr;
					...
					//INS_flag addAction["<t size='1.5' color='#ED2744'>Aller dans le MHQ 1</t>","scripts\Teleporter_BLUFOR_MHQ_1.sqf"];
					//INS_flag addAction["<t size='1.5' color='#ED2744'>Aller dans le MHQ 2</t>","scripts\Teleporter_BLUFOR_MHQ_2.sqf"];
					//INS_flag addAction["<t size='1.5' color='#ED2744'>Aller dans le MHQ 3</t>","scripts\Teleporter_BLUFOR_MHQ_3.sqf"];
					//INS_flag addAction ["<t size='1.5' shadow='2' color='#12F905'>HMS Proteus</t>","scripts\Teleporter_HMS_Proteus.sqf"];
				};
	...
	else
	{
		If (side player == east) then {
			...
			0 spawn {
				...
				*/
				...
				if (JIG_MHQ_enabled) then {
					private ["_op4","_mhqObj","_mhqPos","_mhqObj1","_mhqObj2"];
					missionNamespace setVariable ["INS_usesOP4mhq", true];
					_mhqObj = objNull;
					_mhqObj1 = objNull;
					_mhqObj2 = objNull;
		...
		If (side player == west) then {	
			0 spawn {if (INS_full_loadout in [1,2]) then {loadout = getUnitLoadout player};};
			//if (INS_full_loadout in [1,2]) then {loadout = getUnitLoadout player};};
