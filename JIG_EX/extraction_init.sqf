...
JIG_EX_Caller = 		{TL1};							// Name of playable unit in editor who can call in Extraction Via scroll action, ie; s1, TeamLeader or TL1. This edit is required at the very least to run script pack.
JIG_EX_Chopper_Type	= 	["maestro_B_Heli_Transport_03_F"];		// Desired Evac Chopper classname in quotes and array.
...
JIG_EX_Spawn_Dis = 		4000;							// Chopper spawns at this distance from actual evac marker and flies in.
...
JIG_EX_Smoke_Color = 	"SmokeShellYellow";				// Possible Smoke colors classname in quotes goes here, ie: "SmokeShellOrange","SmokeShellBlue","SmokeShellPurple","SmokeShellGreen","SmokeShellRed","SmokeShell".
...
JIG_EX_damage	=		true;							// true allows damage to be taken / false prohibits damage to be taken.
...
JIG_EX_AmbRadio		=	false;							// Ambient Heli Radio Chatter. Set true to enable. Set false to disable.
...
JIG_EX_gunners		= 	true;							// Set false to make side gunners position empty and available to passengers. Set true to spawn chopper with side gunners/crew with default bis_fnc_spawnvehicle. "false" value tested to support "B_Heli_Transport_01_camo_F" GhostHawk.
