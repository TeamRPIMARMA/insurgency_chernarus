/*
 ...
 jig_ex_actid_show = _ex_caller addAction [("<t color='#12F905'>") + ("Extraction d'hélicoptère") + "</t>", "JIG_EX\extraction_player.sqf", JIG_EX_Caller removeAction jig_ex_actid_show, 1, false, true, "","player ==_target"];
...
if ({_x in (items player + assignedItems player)}count ["ItemMap"] < 1) exitWith {hint localize "STR_BMR_missing_map"; player addAction [("<t color='#12F905'>") + (localize "STR_BMR_heli_extraction") + "</t>","JIG_EX\extraction_player.sqf", JIG_EX_Caller removeAction jig_ex_actid_show, 1, false, true, "", "player ==_target"];};
if (_caller != (leader group _caller) || !("bis_dg_reg" in (allvariables group player))) exitWith {
	...
	player addAction [("<t color='#12F905'>") + (localize "STR_BMR_heli_extraction") + "</t>", "JIG_EX\extraction_player.sqf", JIG_EX_Caller removeAction jig_ex_actid_show, 1, false, true, "", "player ==_target"];
};
...
if (!visibleMap) exitwith {hintSilent localize "STR_BMR_heli_extraction_standby"; ctrlActivate ((findDisplay 12) displayCtrl 107); openMap false; player addAction [("<t color='#12F905'>") + (localize "STR_BMR_heli_extraction") + "</t>","JIG_EX\extraction_player.sqf",JIG_EX_Caller removeAction jig_ex_actid_show,1, false, true,"","player ==_target"]};

_actual_ext_pos = [];
_actual_ext_pos = _actual_ext_pos + call extraction_pos_fnc;
if (DebugEnabled isEqualTo 1) then {vehicle player setPos _pPos;};
if (_actual_ext_pos isEqualTo []) exitWith {hint "Heli cannot land at your chosen position"; ctrlActivate ((findDisplay 12) displayCtrl 107); openMap false; player addAction [("<t color='#12F905'>") + (localize "STR_BMR_heli_extraction") + "</t>","JIG_EX\extraction_player.sqf",JIG_EX_Caller removeAction jig_ex_actid_show,1, false, true,"","player ==_target"]};
...
if (DebugEnabled isEqualTo 1) then {vehicle player setPos _pPos;};
if (_actual_drop_pos isEqualTo []) exitWith {hint "Heli cannot land at your chosen position"; ctrlActivate ((findDisplay 12) displayCtrl 107); openMap false; player addAction [("<t color='#12F905'>") + (localize "STR_BMR_heli_extraction") + "</t>", "JIG_EX\extraction_player.sqf", JIG_EX_Caller removeAction jig_ex_actid_show, 1, false, true, "","player ==_target"]};
...
if (markerPos "extractmkr" distance markerPos "dropmkr" < 500) exitWith {hint "Extraction and Drop Position is less then 500 meters apart. Try again."; player addAction [("<t color='#12F905'>") + (localize "STR_BMR_heli_extraction") + "</t>", "JIG_EX\extraction_player.sqf", JIG_EX_Caller removeAction jig_ex_actid_show, 1, false, true, "", "player ==_target"]};
