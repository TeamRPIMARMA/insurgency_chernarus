...
animate_doors_fnc = { 
	params ["_veh"];
	switch (true) do {								      
		case (_veh isKindOf "maestro_B_Heli_Transport_03_F"): {if (_veh doorPhase "Door_rear_source" < 0.5) then {{_veh animateDoor [_x, 1]} forEach ["Door_rear_source"];} else {{_veh animateDoor [_x, 0]} forEach ["Door_rear_source"];}};
		...
		case (_veh isKindOf "yo_B_Heli_Transport_01_camo_F"): {if ((_veh doorPhase "door_R") == 0) then {{_veh animateDoor [_x, 1]} forEach ["door_L","door_R"];} else {{_veh animateDoor [_x, 0]} forEach ["door_L","door_R"];}};
