...
private _stat_grp = [_newZone,4,25] call spawn_Op4_StatDef_Objectifs;
...
while {_uncaped} do {
	...
	if (_fireF isEqualTo 1) then {
		if (daytime > _dusk || daytime < _dawn) then {
			...
			null=[_sfCount,1,220,"white",100,_newZone] spawn Drop_SmokeFlare_fnc;
			_fireF = 2;
		};
	}else{
	...
	private _rwave = [_newZone,_ins_debug,_defcnt,_maxTime] spawn {
		...
		while {makewave} do	{
			...
			if (count curvePosArr > 0) then	{
				...
				null=[_sfCount,0,215,"white",50,_smokePos] spawn Drop_SmokeFlare_fnc;
