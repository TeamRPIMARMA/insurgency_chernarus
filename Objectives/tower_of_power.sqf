...
_stat_grp = [_newZone,8,25] call spawn_Op4_StatDef;
...

{
	...
	false;
} count nearestObjects [objective_pos_logic, INS_lights, 10000];
...
while {_killtower} do
{
	if (!alive _tower) exitWith {
		...
		for [{_i=0},{_i < (count _lights)},{_i=_i+1}] do {
			_lamps = getPosATL objective_pos_logic nearObjects [_lights select _i, 10000];
			sleep 0.01;
			{
