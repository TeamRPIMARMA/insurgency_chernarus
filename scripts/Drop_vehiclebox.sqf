...
private ["_heli", "_reloadtime"];
	...
	// meat and potatoes
	if ( !(isNil "AW_ammoDrop_B_Sapeur") ) exitWith {hint "Le largage n'est pas disponible actuellement"};
	if ((getPos player) select 2 < _minheight) exitWith {hint parseText _toLow};
	if ((getPos player) select 2 > _minheight) then
	{
		AW_ammoDrop_B_Sapeur = false;
		publicVariable "AW_ammoDrop_B_Sapeur";
